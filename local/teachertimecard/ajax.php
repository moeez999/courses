...
// Get parameters
$date = '2025-06-16';//required_param('date', PARAM_TEXT);
...
print_r($teacherid.$start_date.$end_date);
if (!$day_data) {
...
foreach ($day_data['main_sessions'] as $session) {
    ...
    if ($is_removed) {
        ...
        // Add to removed sessions table with full details
        $removed_rows .= "
        <tr>
            <td>
                <input type='checkbox' class='session-checkbox restore-checkbox' name='restore_sessions[]' value='{$session_id}' data-session-id='{$session_id}'>
            </td>
        ...
        <tr>
            <td>
                <input type='checkbox' class='session-checkbox remove-checkbox' name='remove_sessions[]' value='{$session_id}' data-session-id='{$session_id}' style='display:none'>
            </td>
            <td><div class='table-popup-dot'>{$prefix}</div></td>
            <td>{$duration_mins} min</td>
            <td>{$participant_count}/{$student_count}</td>
...
foreach ($day_data['practice_sessions'] as $session) {
    ...
    $duration_mins = round($session->duration_seconds / 60);
    $prefix = count($meeting_parts) > 0 ? strtoupper($meeting_parts[0]) : '';
    $time = date('H:i', $session->activity_time);
...
// Calculate total hours and minutes (only for non-removed sessions)
$total_minutes = 0;
foreach ($day_data['main_sessions'] as $session) {
    $session_id = "session_" . $session->id;
    if (!isset($removed_sessions_data[$session_id])) {
    ...
    <div class='edit-popup'>
        ...
        <div class='edit-popup-calendar'>
            ...
            <div class='edit-popup-calendar-status'>
                ...
                <div class='edit-popup-calendar-label-paid'>Paid</div>
            </div>
        </div>
