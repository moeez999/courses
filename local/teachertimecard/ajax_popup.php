...
foreach ($day_data['main_sessions'] as $session) {
    ...
    if (count($meeting_parts) >= 2) {
        ...
        $meeting_parts = explode(' ', $session->meeting_type);
        $prefix = count($meeting_parts) > 0 ? strtoupper($meeting_parts[1]) : '';
    }
    ...
    if ($is_removed) {
        ...
        <tr>
            <td></td>
            <td><div class='session-dot'>$prefix</div></td>
        ...
        <tr class='{$paid_class}'>
            ...
            <td><div class='session-dot'>$prefix" . ($is_paid ? " ✓" : "") . "</div></td>
            <td>{$duration_mins} min</td>
            <td>{$participant_count}/{$student_count}</td>
...
echo json_encode([
    ...
    <div class='edit-popup'>
        ...
        <div class='edit-popup-control session-controls'>
            ...
            <button id='add_manual_time_for_step1_open_btn' class='add_manual_time_for_step1_open_btn' data-teacherid='{$teacherid}' data-date='{$date}'>Add</button>
                    |
