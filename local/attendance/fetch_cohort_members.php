...
/**
 * Resolve a user's subscription status with a single, global
 * "most-recent record wins" priority across:
 *   - PayPal/Local/Patreon by EMAIL
 *   - PayPal/Local/Patreon by SubID (profile field)
 * If SubID is "Exclusive" or "ExclusiveN", that wins immediately.
 * If nothing found in those sources, fall back to manual_user_registrations.
 *
 * @param stdClass $user  Must have ->id
 * @param string   $email Raw email (case-insensitive)
 * @return string  'active' | 'Suspended' | 'Canceled' | 'Declined' | 'Inactive' | 'Exclusive' | 'ExclusiveN' | 'Expired' | 'Upcoming' | 'NA'
 */
function get_user_subscription_status(stdClass $user, string $email): string {
    global $DB;

    $lowerEmail = core_text::strtolower(trim($email));
    $userSubscriptionStatus = 'NA';

    // ---------- helpers ----------
    $pick_ts = function($row) {
        $fields = [
            'updated_at','created_at','last_payment_at',
            'timeupdated','timemodified','timecreated',
            'start_date','end_date',
            'patron_since','last_charge_date','charged_at'
        ];
        $cands = [];
        foreach ($fields as $f) {
        ...
        if (!$cands && isset($row->id)) return (int)$row->id; // last-resort within table
        return $cands ? max($cands) : 0;
    };

    $norm = function($statusRaw, $source, $extra = null) {
        $s = core_text::strtolower(trim((string)$statusRaw));
        // returns [label, isActiveBool]
        if ($source === 'paypal') {
            if ($s === 'active')    return ['active', true];
            if ($s === 'suspended') return ['Suspended', false];
            if ($s === 'cancelled') return ['Canceled', false];
            return ['Inactive', false];
        }
        if ($source === 'local') {
            $sub_status = (int)$extra; // 1=active, else inactive/canceled
            if ($sub_status === 1) return ['active', true];
            return ['Canceled', false];
        }
        if ($source === 'patreon') {
            if ($s === 'active_patron')   return ['active', true];
            if ($s === 'declined_patron') return ['Declined', false];
            if ($s === 'former_patron')   return ['Inactive', false];
            return ['Inactive', false];
        }
        return ['Inactive', false];
    };

    $events = []; // will hold BOTH email-based and SubID-based events
...
$grace = defined('DAYSECS') ? (3 * DAYSECS) : (3 * 86400);

foreach ($paypalSubs as $sub) {
    [$label, $isActive] = $norm($sub->status ?? '', 'paypal');

    // ts = end_date (Y-m-d H:i:s) converted to unix + 3 days
    $ts = 0;
    if (!empty($sub->end_date)) {
        $endTs = strtotime((string)$sub->end_date); // e.g. "2025-09-01 11:01:08"
        if ($endTs !== false) {
            $ts = $endTs + $grace;
        }
    }
    ...
    $events[] = [
        'source' => 'paypal',
        'via'    => 'email',
        'ts'     => $ts,            // direct, not using $pick_ts
        'label'  => $label,
        'active' => $isActive
    ];
}
    ...
    // Local by email
    $localSubs = $DB->get_records_sql("
        SELECT *
          FROM {local_subscriptions}
         WHERE LOWER(sub_email) = :email
         ORDER BY id DESC
    ", ['email' => $lowerEmail]) ?? [];
    foreach ($localSubs as $sub) {
        $substatus = isset($sub->sub_status) ? (int)$sub->sub_status : 0;
        [$label, $isActive] = $norm($substatus === 1 ? 'active' : 'canceled', 'local', $substatus);
        $events[] = ['source'=>'local','via'=>'email','ts'=>(int)$sub->sub_modified,'label'=>$label,'active'=>$isActive];
    }
...
foreach ($patreonSubs as $sub) {
    [$label, $isActive] = $norm($sub->status ?? '', 'patreon');

    // ts = enddate converted to unix + 3 days (directly; do not use $pick_ts)
    $ts = 0;
    if (!empty($sub->enddate)) {
        if (is_numeric($sub->enddate)) {
            $endTs = (int)$sub->enddate;
        } else {
            $endTs = strtotime((string)$sub->enddate); // e.g. "2025-06-09"
        }
        if ($endTs !== false && $endTs !== null) {
            $ts = $endTs + $grace;

            // If active but enddate + 3 days has passed, force Inactive
            // if ($isActive && $ts < $now) {
            //     $label = 'Inactive';
            //     $isActive = false;
            // }
    ...
    // ---------- SubID profile lookup ALWAYS (to handle different payer email) ----------
    $fieldid = $DB->get_field('user_info_field', 'id', ['shortname' => 'SubID']);
    $subIDField = $fieldid ? $DB->get_record('user_info_data', ['userid'=>$user->id,'fieldid'=>$fieldid]) : null;

    if ($subIDField && !empty($subIDField->data)) {
        $subscriberId = trim($subIDField->data);

        // Exclusive overrides everything immediately
        if ($subscriberId === 'Exclusive') {
            return 'Exclusive';
        }
        if (preg_match('/^Exclusive(\d+)$/', $subscriberId, $m)) {
            return 'Exclusive' . $m[1];
        }

        // PayPal by subscription_id
        $paypalById = $DB->get_records_sql("
            SELECT *
              FROM {paypal_subscriptions}
             WHERE subscription_id = :subid
             ORDER BY id DESC
        ", ['subid' => $subscriberId]) ?? [];
        foreach ($paypalById as $sub) {
            [$label, $isActive] = $norm($sub->status ?? '', 'paypal');
            $events[] = ['source'=>'paypal','via'=>'subid','ts'=>$pick_ts($sub),'label'=>$label,'active'=>$isActive];
        }

        // Local by sub_reference (collect all for recency)
        $localById = $DB->get_records_sql("
            SELECT *
              FROM {local_subscriptions}
             WHERE sub_reference = :sub_reference
             ORDER BY id DESC
        ", ['sub_reference' => $subscriberId]) ?? [];
        foreach ($localById as $sub) {
            $substatus = isset($sub->sub_status) ? (int)$sub->sub_status : 0;
            [$label, $isActive] = $norm($substatus === 1 ? 'active' : 'canceled', 'local', $substatus);
            $events[] = ['source'=>'local','via'=>'subid','ts'=>$pick_ts($sub),'label'=>$label,'active'=>$isActive];
        }

        // Patreon by subscriber_id
        $patreonById = $DB->get_records_sql("
            SELECT *
              FROM {membership_patreon_subscriptions}
             WHERE subscriber_id = :subid
             ORDER BY id DESC
        ", ['subid' => $subscriberId]) ?? [];
        foreach ($patreonById as $sub) {
            [$label, $isActive] = $norm($sub->status ?? '', 'patreon');
            $events[] = ['source'=>'patreon','via'=>'subid','ts'=>$pick_ts($sub),'label'=>$label,'active'=>$isActive];
        }
    }

    // ---------- Decide by single most-recent across BOTH paths ----------
    if (!empty($events)) {
        usort($events, fn($a,$b) => $b['ts'] <=> $a['ts']); // newest first
        $mostRecent = $events[0];
        $userSubscriptionStatus = $mostRecent['label'];

        // If you prefer: any non-active most-recent => generic 'Inactive', uncomment:
        // if (!$mostRecent['active']) $userSubscriptionStatus = 'Inactive';

        return $userSubscriptionStatus;
    }

    // ---------- Final fallback: manual_user_registrations ----------
    ...
    foreach ($manualSubs as $sub) {
        ...
        $status = core_text::strtolower((string)$sub->status);
        ...
        if ($status === 'active') {
            if ($start <= $now && $now <= $end) {
                ...
                $DB->execute("
                    UPDATE {manual_user_registrations}
                       SET status = 'inactive'
...
$email = strtolower(trim($user->email));
	if($email === 'francoevairene5750@gmail.com')
    {
        echo 'in';
    }
$userSubscriptionStatus = 'NA';

//$userSubscriptionStatus = get_user_subscription_status($user, $email);

require_once($CFG->libdir . '/accesslib.php');
  // ... you have $USER or $userid already
$check = membership_check_user_subscription($user->id); // or your $userid

$subscriptionIsDeclined = ($check['state'] === 'declined');
$hasActiveSubscription  = ($check['state'] === 'active');

$userSubscriptionStatus = $check['state'];
...
// if ($userSubscriptionStatus === 'NA' || 'Canceled' || 'Suspended') {
...
//$userSubscriptionStatus = get_user_subscription_status($userr, $email);

require_once($CFG->libdir . '/accesslib.php');
  // ... you have $USER or $userid already
$check = membership_check_user_subscription($userr->id); // or your $userid
