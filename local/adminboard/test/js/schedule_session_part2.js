...
function addAssignmentChip() {
  ...
  var moduleId = $('#assignmentDropdownSelected').attr('data-module-id');
  ...
  if (assignment !== '' && assignment !== 'Assignment' && startDate && dueDate) {
    ...
    if (!alreadyExists) {
      var chipHtml =
        `<div class="custom-chip-bar assignment-chip" data-chip-key="${key}" data-module-id="${moduleId}" 
     data-startdate="${fullStart}" 
     data-duedate="${fullDue}"  style="margin-bottom: 8px;">
  ...
  $('#assignmentDropdownList').on('click', '.dropdown-item', function() {
    ...
    $('#assignmentDropdownSelected').attr('data-module-id', $(this).attr('data-module-id'));
    closeDropdownToBody('#assignmentDropdownWrapper', '#assignmentDropdownList');
    // No chip added here
  });
...
$(function () {
  const now = new Date();
  let startDate = new Date(now.getFullYear(), now.getMonth(), now.getDate()); // e.g. today at local midnight
...
$(function () {
  const now = new Date();
  let dueDate = new Date(now.getFullYear(), now.getMonth(), now.getDate()); // e.g. today at local midnight
