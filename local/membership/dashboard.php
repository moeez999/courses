...
require_once("membership_manual_subscription_update.php");
}
  ...
  $jq(document).ready(function() {
    ...
    // }
       ...
       buttons += '<button type="button" data-id="' + row.id + '" class="btn btn-primary confirm-cancel-action"><i class="glyphicon glyphicon-remove-circle"></i></button>';
            ...
            if (row.method === 'zelle' && row.status !== '0') {
 
               if (isAdmin || isManager) {
                buttons += '<button type="button" data-id="' + row.id + '" class="btn btn-warning open-update-subscription" title="Update Subscription"><i class="glyphicon glyphicon-edit"></i></button>';
              }
            }
            return `
...
$jq(document).on('click', '.open-update-subscription', async function (e) {
  e.preventDefault();

  // 1) Resolve the clicked row across responsive tables
  let $tr = $jq(this).closest('tr');
  if ($tr.hasClass('child')) $tr = $tr.prev(); // handle responsive
  const data = table.row($tr).data();
  if (!data) return;

  // 2) Cache the Update modal root
  const $update = $jq('#subscriptionUpdateModalBackdrop');

  // 3) Split "name" into first/last (fallback if separate fields are absent)
  let first = '', last = '';
  if (data.name) {
  ...
  // 4) Fill basics
  $update.find('#update_row_id').val(data.id ?? '');
  $update.find('#firstName').val(first);
  $update.find('#lastName').val(last);
  $update.find('#email').val(data.email ?? '');
  $update.find('#password').val(''); // keep blank on update

  // 5) Payment method → lowercase value expected by your <select>
  const method = String(data.method ?? '').toLowerCase();
  $update.find('#paymentMethod').val(method).trigger('change'); // trigger for Select2

  // 6) Billing interval parsing: "1 Month", "2 Weeks", "1 Year" etc.
  (function setIntervalFields(bf) {
    const m = String(bf ?? '').match(/^(\d+)\s+([A-Za-z]+)/);
    let value = 1, unit = 'Month';
    if (m) {
      value = parseInt(m[1], 10) || 1;
      const u = m[2].toLowerCase();
      if (u.startsWith('week')) unit = 'Week';
      else if (u.startsWith('month')) unit = 'Month';
      else if (u.startsWith('year')) unit = 'Year';
    }
    $update.find('#intervalValue').val(value);
    $update.find('#intervalType').val(unit).trigger('change');
  })(data.billingFrequency);

  // 7) Price
  $update.find('#price').val(data.price ?? '');

  // 8) Cohort: your DataTable gives "cohort" as SHORTNAME. Convert to idnumber.
  //    If your #cohort <select> expects the idnumber as its option value,
  //    call the endpoint and set the field on success.
// Inside your existing handler, replace the cohort fetch block with this:
try {
  const shortname = String(data.cohort ?? '').trim();
  const emailForPhone = String(data.email ?? '').trim();

  const body =
    'cohortid=' + encodeURIComponent(shortname) +
    '&email='   + encodeURIComponent(emailForPhone) +
    (typeof M !== 'undefined' && M.cfg && M.cfg.sesskey
      ? '&sesskey=' + encodeURIComponent(M.cfg.sesskey)
      : '');

  const res = await fetch('getCohortIdNumber.php', {
    method: 'POST',
  ...
  // Cohort idnumber into the field/select
  if (j && j.ok && j.idnumber) {
    $update.find('#cohort').val(String(j.idnumber)).trigger('change');
  } else {
    $update.find('#cohort').val('').trigger('change');
  }

  // Phone into #contactNumber (overwrite whatever was there)
  if (j && typeof j.phone === 'string' && j.phone.trim() !== '') {
    $update.find('#contactNumber').val(j.phone.trim());
  } else {
    // keep whatever you already set earlier from table data
    // or clear if you prefer:
    // $update.find('#contactNumber').val('');
  }
} catch (err) {
  console.error('Cohort/phone lookup failed', err);
  $update.find('#cohort').val('').trigger('change');
  // optionally leave contactNumber as-is
}

  // 9) Subscriber/payment refs
  $update.find('#subscriberId').val(data.subscriber_id ?? '');
  $update.find('#paymentReference').val(data.paymentReference ?? data.payment_reference ?? '');

  // 10) Dates (use your preferred format; these set raw values)
  $update.find('#startDate').val(data.startDate ?? '');
  $update.find('#endDate').val(data.endDate ?? '');

  // 11) Referral + notes
  $update.find('#referralCode').val(data.referralcode ?? '');
  $update.find('#notes').val(data.notes ?? '');

  // 12) Status → normalize and update status UI/hidden input
  const raw = String(data.status ?? '').toLowerCase();
  const normalized =
    raw === 'inactive' || raw === '0' ? 'Inactive' :
    raw === 'paused' ? 'Paused' :
    'Active';

  if (typeof setCustomStatusUI === 'function') {
    setCustomStatusUI(normalized);
  } else {
    $update.find('#customStatus').val(normalized);
  }
    ...
    // Request new code button
      ...
      $jq('#saveCohorts').on('click', function() {
        ...
        if ($jq('#instantToggle').is(':checked')) {
          table.cell(rowIndex, 9).data(cohortNames).draw();
          table.cell(rowIndex, 10).data(selectedCohortIds).draw();
        } else {
          ...
          table.cell(rowIndex, 9).data(newCohortInfo).draw();
          table.cell(rowIndex, 10).data(selectedCohortIds).draw();
        }
       ...
       $jq.ajax({
          ...
          success: function(response) {
            ...
            if (response.success) {
              ...
              table.cell(rowIndex, 9).data(originalCohortNames).draw();
              table.cell(rowIndex, 10).data(originalCohortIds).draw();
            }
          },
