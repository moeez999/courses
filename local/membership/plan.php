...
if ($userid) {
  ...
  if (!empty(get_config('local_membership', 'noofmembershipplans'))) {
    $availableCycles = [];
    ...
    for ($key = 1; $key <= $noOfMembershipPlans; $key++) {
              ...
              // 🟢 Cohort-based filtering logic
    if ($userid) {
        $cohortsSetting = get_config('local_membership', 'membershipcohorts' . $key);
        if (empty($cohortsSetting)) {
            continue; // Skip this plan, no cohorts assigned
        }
        $planCohortIds = explode(',', $cohortsSetting);

        $match = false;
        foreach ($userCohortIds as $userCohortId) {
            if (in_array($userCohortId, $planCohortIds)) {
                $match = true;
                break;
            }
        }
        if (!$match) {
            continue; // User not in this plan's cohorts, skip it
        }
    }
    ...
    <div class="row">
      <div class="col-12">
        <div class="card text-center">
          ...
          <div class="card-body">
            <p>Actualmente no hay planes, vuelve luego.</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <?php if (!empty(get_config('local_membership', 'noofmembershipplans'))) { ?>
        <div class="swiper-wrapper">
          ...
          for ($key = 1; $key <= get_config('local_membership', 'noofmembershipplans'); $key++) {
                      ...
                      // 🟢 Apply cohort-based filtering logic (just like in the first loop)
    if ($userid) {
        $cohortsSetting = get_config('local_membership', 'membershipcohorts' . $key);
        if (empty($cohortsSetting)) {
            continue; // Skip this plan, no cohorts assigned
        ...
        if (!$match) {
            ...
            <div class="swiper-slide mb-3 membership-tier" data-all="true"
                  ...
                  <center>
                    <h2 class="plan-title">
                      ...
                      <?php if ($planData->planDiscounts->biannually > 0): ?>
                        <span class="discount-tag hidden" data-cycle="biannually">Ahorras
                          <?= $planData->planDiscounts->biannuallyText ?></span>
                      <?php endif; ?>
                      <?php if ($planData->planDiscounts->yearly > 0): ?>
                        <span class="discount-tag hidden" data-cycle="yearly">Ahorras
                          <?= $planData->planDiscounts->yearlyText ?></span>
                      <?php endif; ?>
                    </h2>
                    <?php if (!$planData->isFree) { ?>
                      <?php if ($planData->isMonthly) { ?>
                        <div class="membership-tier-payment-details hidden" data-cycle="monthly">
                          <p class="price mb-1">
                            $<?= $planData->monthly->price ?> <small> / <?= $planData->monthly->interval ?>
                              <?= $planData->monthly->keyword ?></small>
                          </p>
                          <?php if ($planData->planDiscounts->yearly > 0 && $planData->planDiscounts->biannually > 0): ?>
                            <p class="discount mb-1">
                              *Ahorra con plan anual (<?= $planData->planDiscounts->yearlyText ?>) o semestral
                              (<?= $planData->planDiscounts->biannuallyText ?>)
                            </p>
                      ...
                      <?php if ($planData->isBiannually) { ?>
                        <div class="membership-tier-payment-details hidden" data-cycle="biannually">
                          ...
                          <?php endif; ?>
                          <p class="price mb-1">
                            $<?= $planData->biannually->price ?> <small> / <?= $planData->biannually->interval ?>
                              <?= $planData->biannually->keyword ?></small>
                          </p>
                          <?php if ($planData->planDiscounts->yearly > 0): ?>
                            <p class="discount mb-1">
                              *Ahorra con plan anual (<?= $planData->planDiscounts->yearlyText ?>)
                      ...
                      <?php if ($planData->isYearly) { ?>
                        <div class="membership-tier-payment-details hidden" data-cycle="yearly">
                          ...
                          <?php endif; ?>
                          <p class="price mb-1">
                            $<?= $planData->yearly->price ?> <small> / <?= $planData->yearly->interval ?>
                              <?= $planData->yearly->keyword ?></small>
                          </p>
                        </div>
                      <?php } ?>
                    <?php } else { ?>
