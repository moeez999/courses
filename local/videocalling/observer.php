...
function renderRooms(data) {
  ...
  if (roomsListEl && !__roomsListBound) {
    ...
    roomsListEl.addEventListener('click', async (ev) => {
      ...
      /* ====== NUEVO: si ya estás dentro o en proceso, ignora ====== */
      if (isJoined || isJoining) {
        notify('Ya estás en una llamada. Sal antes de cambiar de sala.', 'info');
        return;
      }
      ...
      try {
        ...
        btn.disabled = false;
        // isJoining se limpia dentro de leave o tras join fallido
      }
    });
  }
}
...
async function joinRoomAsAdmin(roomId) {
  /* ====== NUEVO: guardas anti-doble ejecución ====== */
  //if (isJoined || isJoining) {
  //  notify('Ya estás en una llamada o entrando a una. Primero sal.', 'info');
  //  return;
  //}
  isJoining = true;
  q('leave-btn').disabled = false;
