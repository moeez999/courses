...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function getDeviceId() {
        localStorage.setItem('userId',navigator.userAgent + navigator.deviceMemory + navigator.hardwareConcurrency + moodleID)
        return btoa(navigator.userAgent + navigator.deviceMemory + navigator.hardwareConcurrency + moodleID);
    } 
    ...
    const joinStream = async () => {
        ...
        const response = await fetch(`${api}unirse`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId, time, username, lastRoom, userBlocks, moodleID, scorePro, cohortAbre, eventid}),
        });
    ...
    const handleUserJoined = async (user, mediaType) => {
        ...
        if(clockIniciado === false){
            // if(dataAdminFetch.status !== 200 && dataAdminFetch.status !== 404) return
            
            let dataAdminFetch = await fetch(`${api}verify/admin/${user.uid}`,
                {
                    method: 'GET',
                    headers: { 'Content-Type': 'application/json' },
                }
            )
            let responseAdmin = await dataAdminFetch.json();
            if(responseAdmin.admin) return
