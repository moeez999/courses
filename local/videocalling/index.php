...
$from = $now;
$to = $now;
$tz = new DateTimeZone($timezone); // p.ej. 'America/Sao_Paulo'
$utc = new DateTimeZone('UTC');

$todayLocal = new DateTimeImmutable('today', $tz); // hoy a las 00:00 en TZ del usuario
$startOfDayUtc = $todayLocal->setTimezone($utc)->getTimestamp(); // 00:00 local -> UTC epoch
...
$sql = "SELECT 
        ...
        WHERE b.finishdate >= :start_of_day_utc
        AND b.startdate <= :end_of_day_utc";
$params = [
...
if($datateacher && !$isActive){
    foreach($datateacher as $value) {
        ...
        if ($start <= $currentTimestamp && $end >= $currentTimestamp) {
            // verificar por cohort
            $eventid=$value->idplanificaction;
            $isActive = true;   
