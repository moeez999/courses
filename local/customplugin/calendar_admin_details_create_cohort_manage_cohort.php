  ...
  <div class="calendar_admin_details_create_cohort_content tab-content" id="manage_cohort_tab_content"
    ...
    ?>
          ...
          <div class="calendar_admin_details_create_cohort_input_wrapper">
              ...
              <input type="text" id="cohortShortNameInput" class="calendar_admin_details_create_cohort_input"
              ...
              <div>
                  ...
                  <div class="d-flex" id="customTimeFields" style="margin-top: 10px;">
                      <div class="custom-time-pill">
                          <input type="text" class="form-control time-input" value="9:30 am" autocomplete="off" readonly
                  ...
                  <div class="calendar_admin_details_cohort_tab_timezone_wrapper_right">
                      ...
                      <div class="calendar_admin_details_cohort_tab_timezone_dropdown_right">
                          ...
                          <svg class="calendar_admin_details_cohort_tab_timezone_arrow_right" width="18" height="18"
                              viewBox="0 0 20 20">
          ...
          <div class="teacher-block" data-teacher="2">
              <div>
                  ...
                  <div class="d-flex calendar_admin_details_time_right" style="margin-top:10px;">
                      ...
                      <div class="calendar_admin_details_time_right_time-pill">
                          <input type="text" class="form-control calendar_admin_details_time_right_time-input"
                              value="10:30 am" autocomplete="off" readonly style="background-color:#ffffff;" />
                          <div class="calendar_admin_details_time_right_time-dropdown"></div>
                      </div>
                  </div>
...
$(document).on('click', '#cohortDropdownList li, #cohortList li', async function() {
    const $li = $(this);
    const idnumber = $li.data('idnumber');
    if (!idnumber) return;

    console.log('=== STARTING COHORT SELECTION ===');
    console.log('Selected cohort idnumber:', idnumber);

    // Update cohort dropdown
    $('#cohortDropdownBtn').html($li.text().trim() +
    ...
    try {
        ...
        const res = await fetch(url, {
            ...
            body: 'idnumber=' + encodeURIComponent(idnumber) + '&sesskey=' + encodeURIComponent(
                sesskey)
        });
        ...
        alert('Failed to load cohort data: ' + err.message);
    }
});
...
async function prefillCohortForm(cohortData) {
    ...
    if (c.shortname) {
        ...
        $('#cohortShortNameInput').val(c.shortname);
    }

    // 2. Toggle states
    console.log('Setting toggles - enabled:', c.enabled, 'visible:', c.visible);
    $('#toggleActive').toggleClass('active', c.enabled === 1);
    $('#toggleAvailable').toggleClass('active', c.visible === 1);

    // 3. Color - Set both left and right color pickers
    if (c.color) {
        console.log('Setting color:', c.color);
        $('.create_new_cohort_tab_select_color_left_circle').css('background', c.color);
    ...
    if (c.tutor) {
        ...
        await populateTeacherBlock(2, c.tutor, fmtDate, weeklyLabel, c.startdate);
    }
}
...
async function populateTeacherBlock(teacherNum, teacherData, fmtDate, weeklyLabel, startdate) {
    ...
    // Determine selectors based on teacher number
    ...
    // 1. Schedule
    if (teacherData.days) {
        const scheduleText = weeklyLabel(teacherData.days);
        console.log(`Teacher ${teacherNum} schedule:`, scheduleText);
        $(`.teacher-block[data-teacher="${teacherNum}"] .cohort_schedule_btn`).html(
            scheduleText + ' <span class="cohort_schedule_arrow">▼</span>'
        );
    }

    // 2. Times
    const $timeInputs = $(timeInputSelector);
    ...
    if (teacherData.teacher && teacherData.teacher.id) {
        ...
        await selectTeacherFromDropdown(teacherPrefix, teacherData.teacher);
    }

    // 4. Class Name
    const className = teacherData.classname || (isTeacher1 ? 'Main Class' : 'Tutoring Class');
    console.log(`Setting class name: ${className}`);
    $(`#${classPrefix}DropdownBtn`).html(
        className + ' <svg viewBox="0 0 20 20"><path d="M7 8l3 3 3-3"></path></svg>'
    );
    ...
    if (startdate) {
        ...
        $(`.teacher-block[data-teacher="${teacherNum}"] .conference_modal_date_btn`).text(dateText);
    }
}
...
function selectTeacherFromDropdown(teacherNumber, teacherData) {
    return new Promise((resolve) => {
        ...
        // Find teacher option with matching user ID
        const $teacherOption = $(`${dropdownListId} li[data-userid="${teacherData.id}"]`);

        if ($teacherOption.length > 0) {
            console.log(`Found matching teacher:`, $teacherOption.text().trim());

            // Get the complete HTML (including avatar image)
            ...
            console.log(`Successfully selected ${teacherNumber}:`, teacherData.name);
        } else {
            console.warn(`Teacher ID ${teacherData.id} not found in dropdown`);

            // Fallback: Create manual HTML
...
function gatherTeacherData(teacherNum) {
    ...
    return {
        id: $(`#${teacherPrefix}UserId`).val(),
        name: $(`#${teacherPrefix}DropdownBtn`).find('span').first().text().trim(),
        className: $(`#${classPrefix}DropdownBtn`).contents().first()[0].textContent.trim(),
        schedule: $(`.teacher-block[data-teacher="${teacherNum}"] .cohort_schedule_btn`).contents().first()[0]
            .textContent.trim(),
...
// ===== TEACHER DROPDOWN CLICK HANDLERS =====
$(document).on('click', '#teacher1DropdownList li.teacher-option', function() {
    const userId = $(this).data('userid');
    const teacherHtml = $(this).html();

    $('#teacher1DropdownBtn').html(
        teacherHtml +
        '<svg viewBox="0 0 20 20"><path d="M7 8l3 3 3-3"></path></svg>'
    );
    $('#teacher1UserId').val(userId);
    $('#teacher1DropdownList').hide();
});

$(document).on('click', '#teacher2DropdownList li.teacher-option', function() {
    const userId = $(this).data('userid');
    const teacherHtml = $(this).html();

    $('#teacher2DropdownBtn').html(
        teacherHtml +
        '<svg viewBox="0 0 20 20"><path d="M7 8l3 3 3-3"></path></svg>'
    );
    $('#teacher2UserId').val(userId);
    $('#teacher2DropdownList').hide();
});

// ===== CLASS NAME DROPDOWN HANDLERS =====
$(document).on('click', '#className1DropdownList li', function() {
    const className = $(this).text().trim();
    $('#className1DropdownBtn').html(
        className + ' <svg viewBox="0 0 20 20"><path d="M7 8l3 3 3-3"></path></svg>'
    );
    $('#className1DropdownList').hide();
});

$(document).on('click', '#className2DropdownList li', function() {
    const className = $(this).text().trim();
    $('#className2DropdownBtn').html(
        className + ' <svg viewBox="0 0 20 20"><path d="M7 8l3 3 3-3"></path></svg>'
    );
...
$(document).ready(function() {
    ...
    $('#cohortDropdownBtn').click(function(e) {
        ...
        $('#shortNameDropdownList, #teacher1DropdownList_manage_cohort, #teacher2DropdownList_manage_cohrt, #className1DropdownList_manage_cohort, #className2DropdownList_manage_cohort')
            .hide();
    });
    ...
    $('#shortNameDropdownBtn').click(function(e) {
        ...
        $('#cohortDropdownList, #teacher1DropdownList_manage_cohort, #teacher2DropdownList_manage_cohrt, #className1DropdownList_manage_cohort, #className2DropdownList_manage_cohort')
            .hide();
    });
    ...
    $('#teacher1DropdownBtn_manage_cohort').click(function(e) {
        ...
        $('#cohortDropdownList, #shortNameDropdownList, #teacher2DropdownList_manage_cohrt, #className1DropdownList_manage_cohort, #className2DropdownList_manage_cohort')
            .hide();
    });
    $('#teacher1DropdownList_manage_cohort li').click(function() {
        $('#teacher1DropdownBtn_manage_cohort').html($(this).html() +
            '<svg viewBox="0 0 20 20"><path d="M7 8l3 3 3-3"></path></svg>');
    ...
    $('#teacher2DropdownBtn_manage_cohort').click(function(e) {
        ...
        $('#cohortDropdownList, #shortNameDropdownList, #teacher1DropdownList_manage_cohort, #className1DropdownList_manage_cohort, #className2DropdownList_manage_cohort')
            .hide();
    });
    $('#teacher2DropdownList_manage_cohrt li').click(function() {
        $('#teacher2DropdownBtn_manage_cohort').html($(this).html() +
            '<svg viewBox="0 0 20 20"><path d="M7 8l3 3 3-3"></path></svg>');
    ...
    $('#className1DropdownBtn_manage_cohort').click(function(e) {
        ...
        $('#cohortDropdownList, #shortNameDropdownList, #teacher1DropdownList_manage_cohort, #teacher2DropdownList_manage_cohrt, #className2DropdownList_manage_cohort')
            .hide();
    });
    ...
    $('#className2DropdownBtn_manage_cohort').click(function(e) {
        ...
        $('#cohortDropdownList, #shortNameDropdownList, #teacher1DropdownList_manage_cohort, #teacher2DropdownList_manage_cohrt, #className1DropdownList_manage_cohort')
            .hide();
    });
    ...
    $('#conferenceTeachersDropdownList li').click(function() {
        ...
        $('#conferenceTabContent .conference_modal_attendee').each(function() {
            if ($(this).find('.conference_modal_attendee_name').text().trim() === name) exists =
                true;
