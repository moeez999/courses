...
class participants extends \table_sql implements dynamic_table {
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        $sort = $this->get_sql_sort();
         $sort = preg_replace('/^cohorts\b/', 'email', $sort);
        if ($sort) {
            $sort = 'ORDER BY ' . $sort;
        }
        ...
        foreach ($rawdata as $user) {
           ...
           // Check if cohorts are not empty and assign the idnumber of the first cohort.
            $user->cohort = !empty($cohorts) ? reset($cohorts)->idnumber : null;
