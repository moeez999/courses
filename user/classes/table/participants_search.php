...
use context_course;  // Add this at the top of your file
...
class participants_search {
    ...
    int $limitfrom = 0, int $limitnum = 0): moodle_recordset {
        ...
        // Get the course context (replace with the actual course ID if needed)
        $context = context_course::instance($this->course->id);
    
        // Get the roles assigned to the user in the course context
        $roles = get_user_roles($context, $USER->id, false);
    
        // Flag to check if the user is a student
        $is_enrolled_as_student = false;
    
        // Check if the user has the 'student' role
        foreach ($roles as $role) {
            if ($role->shortname == 'student') { // Adjust role name or ID if necessary
                $is_enrolled_as_student = true;
                break;
            }
        }
    
        // If the user is a student, restrict the participants to their cohorts
         if ($is_enrolled_as_student && !is_siteadmin($USER->id)) {
            // Get the cohorts the user belongs to
            ...
            if (empty($cohortids)) {
                // If the user is not part of any cohort, return an empty result
                return $DB->get_recordset_sql("SELECT * FROM {user} WHERE 1=0"); // Empty recordset
            }
    
            // Prepare the SQL query to filter participants based on their cohorts
            $cohort_ids = implode(',', $cohortids);  // Convert the cohort IDs into a comma-separated string
    
            // Generate SQL components for cohort-based participants
            [
                'subqueryalias' => $subqueryalias,
                'outerselect' => $outerselect,
                'innerselect' => $innerselect,
               ...
               // SQL query to get participants
    $sql = "
    SELECT DISTINCT u.id, u.firstname, u.lastname, u.email
    FROM {user} u, {cohort_members} cm
    WHERE u.id = cm.userid
    AND cm.cohortid IN ($cohort_ids)
    {$sort}
";

// Return the recordset
return $DB->get_recordset_sql($sql, [], $limitfrom, $limitnum);
        ...
        // SQL query for non-students (or all participants)
        $sql = "{$outerselect}
                          FROM ({$innerselect}
                                          FROM {$innerjoins}
                                 {$innerwhere}
        ...
        // Execute the query and return the participants
        return $DB->get_recordset_sql($sql, $params, $limitfrom, $limitnum);
    }
}
    ...
    public function get_total_participants_count(string $additionalwhere = '', array $additionalparams = []): int {
       ...
       // Get the course context (replace with the actual course ID if needed)
$context = context_course::instance($this->course->id);  // Correct context_course usage
...
// Check if the user has the 'student' role (assuming 'student' role is ID 5, adjust if needed)
foreach ($roles as $role) {
    if ($role->shortname == 'student') { // Adjust role name or ID if necessary
        $is_enrolled_as_student = true;
        break;
...
if ($is_enrolled_as_student && !is_siteadmin($USER->id)) {
    ...
    if (empty($cohortids)) {
        // If the user is not part of any cohort, return 0
        return 0;
    }

    // Now, get the count of participants in these cohorts
    // Prepare a SQL query to count the users in these cohorts
    global $DB;
    $cohort_ids = implode(',', $cohortids);  // Convert the cohort IDs into a comma-separated string

    // Query to count users who are part of the given cohorts
    $sql = "
        SELECT COUNT(u.id) 
        FROM {user} u
        JOIN {cohort_members} cm ON u.id = cm.userid
        WHERE cm.cohortid IN ($cohort_ids)
    ";
