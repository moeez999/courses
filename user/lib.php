...
$avatarPath = __DIR__ . '/avtar.jpeg'; // file placed inside /user folder
...
if (file_exists($avatarPath)) {
    ...
    // Remove any existing icons (precaution)
    $fs->delete_area_files($usercontext->id, 'user', 'icon');

    // File record for Moodle
    $filerecord = [
        ...
        'itemid'    => 0,
        'filepath'  => '/',
        'filename'  => 'f1.png' // Must be f1/f2/f3.png per Moodle avatar spec
    ];

    // Upload the JPEG using the expected filename (even if internally it's a JPEG)
    $fs->create_file_from_pathname($filerecord, $avatarPath);

    // Mark that this user has a custom profile picture
    $DB->set_field('user', 'picture', 1, ['id' => $newuserid]);
}
...
function user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    require_once($CFG->libdir . '/accesslib.php');
        ...
        // Check if the user is a student (5) and **only** student
        $isStudent = (
   ...
   // 1) FETCH PAYPAL: keep your exact SQL but use get_recordset_sql so rows don't collapse
$paypalSubscriptions = [];
$rs = $DB->get_recordset_sql("
    SELECT email, status, price, start_date, end_date, created_at
    FROM {paypal_subscriptions}
    ORDER BY id DESC
");
foreach ($rs as $row) {
    $paypalSubscriptions[] = $row; // preserve ALL rows
}
$rs->close();
...
// 3️⃣ NEW: include manual registrations (join to user for email), normalize status.
$manualSubscriptions = [];
$manualSubsRaw = $DB->get_records_sql("
    SELECT u.email,
           mur.status      AS manual_status,
           mur.subscriber_id,
           mur.paymentmethod,
           mur.payment_reference,
           mur.price,
           mur.userid,
           mur.cohortid,
           mur.start_date,
...
");
$now = time();
foreach ($manualSubsRaw as $msub) {
    $code = core_text::strtolower(trim((string)$msub->manual_status));

    // Normalize to 'active'/'inactive' (also consider end_date expiry).
    if ($code === 'active' || $code === 'approved') {
        $msub->status = 'active';
    } elseif (!empty($msub->end_date) && (int)$msub->end_date < $now) {
        $msub->status = 'inactive';
    } elseif (in_array($code, ['cancelled','canceled','declined','failed','suspended','paused','inactive','expired'], true)) {
        $msub->status = 'inactive';
    } else {
        $msub->status = 'inactive';
    }

    unset($msub->manual_status);
    $msub->source = 'manual';
    $manualSubscriptions[] = $msub;
}

$paypalSubscriptions   = $paypalSubscriptions   ?? [];
$localSubscriptions    = $localSubscriptions    ?? [];
$patreonSubscriptions  = $patreonSubscriptions  ?? [];
$manualSubscriptions   = $manualSubscriptions   ?? [];

// ✅ Merge all subscription arrays (PayPal + Local + Patreon).
$allSubscriptions = array_merge($paypalSubscriptions, $localSubscriptions, $patreonSubscriptions,  $manualSubscriptions);

//$allSubscriptions = array_merge($paypalSubscriptions);
    
    
    
    // foreach ($allSubscriptions as $subscription) {
    //   if (strtolower(trim((string)$subscription->email)) === strtolower(trim((string)$user->email))) {
    ...
    // ---------------- PRIORITIZED DECISION (active > inactive > declined; newest wins) ----------------
$subscriptionIsActive   = false;    // <- make sure this is defined before use
$subscriptionIsDeclined = false;    // optional if you need it elsewhere
$forcedSubscriptionId   = '';       // optional
$price                  = null;     // optional

$targetEmail = core_text::strtolower(trim((string)$user->email));

// status priority (higher is better)
$statusPriority = ['active' => 3, 'inactive' => 2, 'declined' => 1];

// pick a usable timestamp from any available fields
$pickTs = function($r) {
    $candidates = [
        $r->updated_at ?? null,
        $r->modified_at ?? null,
        ...
        $r->timecreated ?? null,
        $r->timemodified ?? null,
        $r->id ?? null, // last resort proxy for recency
    ];
...
// collect only this user's records, normalize, attach timestamp
$normalized = [];
$seq = 0;
foreach ($allSubscriptions as $r) {
    if (empty($r->email)) continue;
    if (core_text::strtolower(trim((string)$r->email)) !== $targetEmail) continue;

    $r->status = isset($r->status) ? $normalizeStatus($r->status) : 'inactive';
    $r->__ts   = $pickTs($r);
    $r->__seq  = $seq++; // stable tiebreaker
    $normalized[] = $r;
}

// choose best by status, then most recent
$best = null;
foreach ($normalized as $r) {
    if ($best === null) { $best = $r; continue; }
    $bp = $statusPriority[$best->status] ?? 0;
    $rp = $statusPriority[$r->status] ?? 0;

    if ($rp > $bp
        || ($rp === $bp && ($r->__ts > $best->__ts || ($r->__ts === $best->__ts && $r->__seq > $best->__seq)))) {
        $best = $r;
    }
}

// set the flags your page needs
if ($best) {
    // print_r($best);
    // die;
    if ($best->status === 'active') {
        $subscriptionIsActive = true;
    } elseif ($best->status === 'declined') {
        $subscriptionIsDeclined = true;
        if (!empty($best->sub_reference)) $forcedSubscriptionId = $best->sub_reference;
    }
}


// ---------- FALLBACK: use profile field "SubID" ONLY IF NOT ACTIVE ----------
if (!$subscriptionIsActive) {
    // read profile field SubID
    $subidfieldid = $DB->get_field('user_info_field', 'id', ['shortname' => 'SubID']);
    $subid = $subidfieldid ? $DB->get_field('user_info_data', 'data', [
        'userid'  => $user->id,
        'fieldid' => $subidfieldid
    ]) : null;

    if (!empty($subid)) {
        
        // Clean SubID from profile before lookups
$cleanSubid = (string)$subid;
$cleanSubid = str_replace(["–","—","−"], "-", $cleanSubid); // normalize dashes
$cleanSubid = preg_replace('/[\x{00}-\x{1F}\x{7F}\x{A0}\x{2000}-\x{200B}\x{FEFF}]/u', '', $cleanSubid); // strip invisibles
$cleanSubid = trim($cleanSubid);
        // 1) PayPal by subscription_id
        $pp = $DB->get_record('paypal_subscriptions',
            ['subscription_id' => $cleanSubid],
            'status, price, start_date, end_date, created_at'
        );

        if ($pp) {
            $ppstatus = core_text::strtolower(trim((string)$pp->status));
            if ($ppstatus === 'active') {
                $subscriptionIsActive = true;
                if (isset($pp->price) && $pp->price !== '' && $pp->price !== null) {
                    $price = (float)$pp->price;
                }
            } elseif ($ppstatus === 'declined') {
                $subscriptionIsDeclined = true;
                // $forcedSubscriptionId = $subid; // set if you want to force retry flow
                if (isset($pp->price) && $pp->price !== '' && $pp->price !== null) {
                    $price = (float)$pp->price;
                }
            } else { // inactive/cancelled/etc.
                if (isset($pp->price) && $pp->price !== '' && $pp->price !== null) {
                    $price = (float)$pp->price;
                }
            }
        ...
        // 2) Patreon by subscriber_id (only if still not active)
        if (!$subscriptionIsActive) {
            $pat = $DB->get_record('membership_patreon_subscriptions',
                ['subscriber_id' => $cleanSubid],
                'status, price'
            );
            if ($pat) {
                $patstatus = core_text::strtolower(trim((string)$pat->status)); // active_patron / former_patron
                if ($patstatus === 'active_patron') {
                    $subscriptionIsActive = true;
                    if ($pat->price !== null && $pat->price !== '') {
                        $price = (float)$pat->price;
                    }
                } elseif ($patstatus === 'former_patron') {
                    // stays inactive
                }
            }
        }

        // 3) Braintree / Local by sub_reference (only if still not active)
        if (!$subscriptionIsActive) {
            $loc = $DB->get_record('local_subscriptions',
                ['sub_reference' => $cleanSubid],
                'sub_status, sub_history'
            );

            if ($loc) {
                // 1=active, 5=declined, others inactive
                $code = (int)$loc->sub_status;
                if ($code === 1) {
                    $subscriptionIsActive = true;
                } elseif ($code === 5) {
                    $subscriptionIsDeclined = true;
                    $forcedSubscriptionId = $cleanSubid;
                }

                // Price from latest entry in sub_history, if present
                if (!empty($loc->sub_history)) {
                    $hist = json_decode($loc->sub_history, true);
                    if (json_last_error() === JSON_ERROR_NONE && is_array($hist)) {
                        for ($i = count($hist) - 1; $i >= 0; $i--) {
                            if (isset($hist[$i]['plan_price']) && $hist[$i]['plan_price'] !== '') {
                                $price = (float)$hist[$i]['plan_price'];
                                break;
                            }
                        }
                    }
...
// ---------------- END PRIORITIZED DECISION ----------------
    ...
    foreach ($customitems as $item) {
        // Disable custom item link if student and not subscribed
    if ($isStudent && !$subscriptionIsActive && isset($item->url)) {
        $item->url = null; // Make it non-clickable
    }
    ...
    // Disable custom item link if student and not subscribed
    if ($isStudent && !$subscriptionIsActive ) {
        // Links: Preferences.
...
// Links: Preferences.
    ...
    if (is_siteadmin() || isset($hasadminlink)) {
        $admincohortlinktitle = get_string('cohorts', 'cohort');
        $admincohortlinkurl = new moodle_url('/cohort/index.php');
        $websiteAdminCohort = new stdClass();
        $websiteAdminCohort->itemtype = 'link';
        $websiteAdminCohort->url = $admincohortlinkurl;
        $websiteAdminCohort->title = $admincohortlinktitle;
        $websiteAdminCohort->titleidentifier = 'Cohorts,moodle';
        $returnobject->navitems[] = $websiteAdminCohort;


        $adminuserslinktitle = 'Users';
        $adminuserslinkurl = new moodle_url('/admin/user.php');
        $websiteAdminUsers = new stdClass();
        $websiteAdminUsers->itemtype = 'link';
        $websiteAdminUsers->url = $adminuserslinkurl;
        $websiteAdminUsers->title = $adminuserslinktitle;
        $websiteAdminUsers->titleidentifier = 'Users,moodle';
        $returnobject->navitems[] = $websiteAdminUsers;
        
        $adminlinktitle = get_string('administrationsite', 'moodle');
        $adminlinkurl = new moodle_url('/admin/search.php');
        $websiteAdmin = new stdClass();
        $websiteAdmin->itemtype = 'link';
        $websiteAdmin->url = $adminlinkurl;
        $websiteAdmin->title = $adminlinktitle;
        $websiteAdmin->titleidentifier = 'siteadministration,moodle';
        $returnobject->navitems[] = $websiteAdmin;
    }
    
        global $USER;

    if (isloggedin() && !isguestuser() && user_has_role_assignment($USER->id, 5) && $subscriptionIsActive ) {  
        // Role ID 5 is usually 'Student' in Moodle
        $deleteaccountlinktitle = 'Delete Account';
        $deleteaccountlinkurl = new moodle_url('/local/deleteaccount/index.php');
        $websiteDeleteAccount = new stdClass();
        $websiteDeleteAccount->itemtype = 'link';
        $websiteDeleteAccount->url = $deleteaccountlinkurl;
        $websiteDeleteAccount->title = $deleteaccountlinktitle;
        $websiteDeleteAccount->titleidentifier = 'Delete Account,moodle';
        $returnobject->navitems[] = $websiteDeleteAccount;
    }

    if (isloggedin() && !isguestuser() && user_has_role_assignment($USER->id, 5) && !$subscriptionIsActive ) {  
        // Role ID 5 is usually 'Student' in Moodle
        $deleteaccountlinktitle = 'Delete Account';
        $deleteaccountlinkurl = null;
        $websiteDeleteAccount = new stdClass();
        $websiteDeleteAccount->itemtype = 'link';
        $websiteDeleteAccount->url = $deleteaccountlinkurl;
        $websiteDeleteAccount->title = $deleteaccountlinktitle;
