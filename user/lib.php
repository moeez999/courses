...
function user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    $localSubscriptionsRaw = $DB->get_records_sql("
        SELECT sub_email AS email, sub_status
        FROM {local_subscriptions}
        ORDER BY id DESC
    ");

    // Convert local_subscriptions to use the same 'status' key, and normalize the status value.
    $localSubscriptions = [];
    foreach ($localSubscriptionsRaw as $sub) {
        $sub->status = ($sub->sub_status == 1) ? 'active' : 'inactive';
        unset($sub->sub_status); // Optional: remove original field.
        $localSubscriptions[] = $sub;
    }

    
    // Merge both subscription arrays.
    $allSubscriptions = array_merge($paypalSubscriptions, $localSubscriptions);
