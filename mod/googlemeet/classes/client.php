...
class client {
    ...
    public function print_user_info($scope = null) {
        ...
        $out .= html_writer::link($logouturl,
            $OUTPUT->pix_icon('i/logout', '', 'googlemeet', ['class' => 'm-0']),
    ...
    public function syncrecordings($googlemeet) {
        ...
        public function syncrecordings_new($googlemeet) {
    ...
    if ($secs < MINSECS) {
        return '0:' . str_pad((string)(int)floor($secs), 2, "0", STR_PAD_LEFT);
    } else if ($secs >= MINSECS && $secs < HOURSECS) {
        return (int)floor($secs / MINSECS) . ':' .
               str_pad((string)(int)floor($secs % MINSECS), 2, "0", STR_PAD_LEFT);
    } else {
        return (int)floor($secs / HOURSECS) . ':' .
               str_pad((string)(int)floor(($secs % HOURSECS) / MINSECS), 2, "0", STR_PAD_LEFT) . ':' .
               str_pad((string)(int)floor(($secs % HOURSECS) % MINSECS), 2, "0", STR_PAD_LEFT);
    }
}
