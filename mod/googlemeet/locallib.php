...
/**
 * Generates a list of users from a specific cohort.
 *
 * @param int $cohortid the cohort ID
 * @return stdClass list of users
 */
function googlemeet_get_users_by_cohort($cohortid) {
    global $DB;
 ...
 * @return void
 */
function googlemeet_send_notification_old($user, $event) {
    global $CFG;
    ...
    $subject = "🎉 Sigamos hablando inglés 🎉 - {$event->coursename}";
...
function googlemeet_send_notification($user, $event) {
    ...
    $starttime = userdate($event->eventdate, '%I:%M %p', $user->timezone); // Time in AM/PM
    

    
    $googlemeet = $DB->get_record('googlemeet', ['id' => $event->googlemeetid], '*', MUST_EXIST);
    $daysspanish = format_days_spanish($googlemeet->days); // Helper function below
   
    $studentname = fullname($user); // Get student's full name
    $meeturl = $event->url;
    $meetname = $event->googlemeetname;

    $htmlmessage = "
<p>Hola <strong>{$studentname}</strong>!</p>

<p>¡Vamos! Únete a la clase en vivo a través de Google Meet 😃:</p>

<p>👉 <a href='{$meeturl}' target='_blank'>Presiona aquí e ingresa a tu clase en vivo</a></p>

<p><strong>{$meetname}</strong> los días <strong>{$daysspanish}</strong> a las <strong>{$starttime}</strong></p>

<p>👉 <a href='https://www.liveworksheets.com/user/login' target='_blank'>Presiona aquí y realiza tus tareas en Liveworksheets</a></p>

<p>¿Necesitas ayuda?</p>

<p>👉 <a href='https://wa.me/message/2AG5XXSSTORZN1' target='_blank'>Presiona Aquí para enviarnos un WhatsApp</a> (todos los países)<br>
Llámanos al: +1 (754) 364-4125 (USA)</p>

<p><strong>Latingles Academy</strong><br>
...
";
    ...
    $message->userto = $user;
    $message->subject = $subject;
    $message->fullmessage = strip_tags($htmlmessage);
    $message->fullmessageformat = FORMAT_HTML;
    $message->fullmessagehtml = $htmlmessage;
    $message->smallmessage = $subject;
    $message->notification = 1;
    $message->contexturl = $meeturl;
    $message->contexturlname = $meetname;
    $message->courseid = $event->courseid;

// if($user->email == 'rktech955@gmail.com')
// {
//   message_send($message);  
// }
    message_send($message);
...
function format_days_spanish($daysjson) {
    $daymap = [
        'Mon' => 'Lunes',
        'Tue' => 'Martes',
        'Wed' => 'Miércoles',
    ...
    if (!$days || !is_array($days)) return '';

    $translated = [];
    foreach ($daymap as $eng => $esp) {
        if (!empty($days[$eng])) {
            $translated[] = $esp;
        }
    }
    ...
    if ($count === 0) {
        ...
        return implode(', ', $translated) . ' y ' . $last;
    }
}
