...
function sync_recordings($googlemeetid, $files) {
    ...
    if ($insertrecordings) {
     ...
     if ((int)$googlemeetrecord->course === 24) {
    global $DB, $PAGE, $CFG;
    require_once($CFG->dirroot . '/user/lib.php'); // fullname(), core_user::get_noreply_user()

    $userids = [];

    // ---------- Helpers ----------
    $collect_profile_emails = function ($node, array &$out) use (&$collect_profile_emails): void {
        ...
        foreach (['c','showc'] as $k) {
            if (!empty($node[$k]) && is_array($node[$k])) {
                foreach ($node[$k] as $child) {
                    $collect_profile_emails($child, $out);
                }
            }
        }
    };

    $decode_availability = function (?string $json) {
        if (empty($json)) return null;
        $arr = json_decode($json, true);
        return is_array($arr) ? $arr : null;
    };

    // ---------- 1) Split emails: STUDENTS (activity) vs TEACHERS (section) ----------
    $studentEmails = [];
    $teacherEmails = [];

    // Activity availability → students
    $acttree = $decode_availability($cm->availability ?? null);
    if ($acttree) {
        $collect_profile_emails($acttree, $studentEmails);
    }

    // Section availability → teachers
    $sectionavailability = $DB->get_field('course_sections', 'availability', ['id' => $cm->section]);
    $sectree = $decode_availability($sectionavailability);
    if ($sectree) {
        $collect_profile_emails($sectree, $teacherEmails);
    }

    // Normalise/dedupe
    $studentEmails = array_values(array_unique(array_map('strtolower', array_filter($studentEmails))));
    $teacherEmails = array_values(array_unique(array_map('strtolower', array_filter($teacherEmails))));

    // ---------- 2) Fetch users ----------
    $students = []; // keyed by id
    $teachers = []; // keyed by id

    if ($studentEmails) {
        list($in_sql, $in_params) = $DB->get_in_or_equal($studentEmails, SQL_PARAMS_NAMED);
        $us = $DB->get_records_sql("
            SELECT id, firstname, lastname, username, email, picture, imagealt, deleted, suspended
              FROM {user}
             WHERE LOWER(email) $in_sql
        ...
        if ($us) {
            foreach ($us as $u) {
                ...
                $upic->size = 100;
                $u->profileimgurl = $upic->get_url($PAGE)->out(false);
    ...
    if ($teacherEmails) {
        list($in_sql, $in_params) = $DB->get_in_or_equal($teacherEmails, SQL_PARAMS_NAMED);
        $ut = $DB->get_records_sql("
            SELECT id, firstname, lastname, username, email, picture, imagealt, deleted, suspended
              FROM {user}
             WHERE LOWER(email) $in_sql
               AND deleted = 0 AND suspended = 0
    ...
    if (!$students && !$teachers) {
        // Nothing to notify; continue normal flow
        return googlemeet_list_recordings(['googlemeetid' => $googlemeetid]);
    }

    // ---------- 3) Build top-3 recordings (your existing logic kept) ----------
    ...
    $activityName = $DB->get_field('googlemeet', 'name', ['id' => $googlemeetid]) ?? '';

    // Render recordings block once to inject in both templates.
    $recordingsHtml = '';
    if (!empty($topRecordings)) {
        $recordingsHtml = "<div style='margin-top:16px'>";
        $recordingsHtml .= "<p style='margin:0 0 6px 0'><strong>Últimas grabaciones:</strong></p>";
        foreach ($topRecordings as $rec) {
            $date = date('F d, Y', (int)$rec->createdtime);
            $name = htmlspecialchars($activityName, ENT_QUOTES);
            $link = htmlspecialchars($rec->webviewlink ?? '', ENT_QUOTES);
            if (!$link) continue;
            $recordingsHtml .= "<p style='margin:4px 0'><strong>{$name}</strong> – {$date}<br>";
            $recordingsHtml .= "<a href='{$link}'>Ver la grabación</a></p>";
        }
        $recordingsHtml .= "</div>";
    }
    ...
    // For student template we need a teacher name to show; pick the first teacher if multiple.
    $primaryTeacherName = $teachers ? reset($teachers)->fullname : 'your teacher';

    $noreplyuser = \core_user::get_noreply_user();

    // ---------- 5) Send to STUDENTS (student template) ----------
foreach ($students as $student) {
    // TEMP: force to your test inbox
    $student->email = 'rktech955@gmail.com';

    $studentname = $student->fullname;
    $teachername = $primaryTeacherName; // you computed this earlier

    // Render top-3 recordings block (HTML)
    $recHtml = '';
    if (!empty($topRecordings)) {
        $recHtml .= "<div style='margin-top:16px'>";
        $recHtml .= "<p style='margin:0 0 6px 0'><strong>Últimas grabaciones:</strong></p>";
        foreach ($topRecordings as $rec) {
            $date = date('F d, Y', (int)$rec->createdtime);
            $link = htmlspecialchars($rec->webviewlink ?? '', ENT_QUOTES);
            if (!$link) { continue; }
            $title = htmlspecialchars($activityName ?? '', ENT_QUOTES);
            $recHtml .= "<p style='margin:4px 0'><strong>{$title}</strong> – {$date}<br>";
            $recHtml .= "<a href='{$link}'>Watch Recording</a></p>";
        }
        $recHtml .= "</div>";
    }
...
<html>
  <body style='font-family: Arial, sans-serif; line-height: 1.6;'>
    ...
    <p>I hope you’re doing well! To help you stay on track, here are the recordings from recent 1:1 classes with <strong>{$teachername}</strong>:</p>
    {$recHtml}
    <p>Please make sure to review these sessions as soon as you can so you don’t fall behind.</p>
    <p>If you have any questions or need clarification on any of the material, feel free to reach out.</p>
    <p>Keep up the great work!</p>
    <p>Best regards,<br>Latingles Academy<br>WhatsApp: +1 754-364-4125</p>
  </body>
</html>";

    // Plain-text fallback (prevents blank mails on some setups)
    $text = "Dear {$studentname},\n"
          . "Here are the latest recordings for your recent 1:1 classes with {$teachername}:\n"
          . implode("\n", array_map(function($r) use($activityName) {
                $date = date('F d, Y', (int)$r->createdtime);
                $link = $r->webviewlink ?? '';
                return "{$activityName} – {$date}: {$link}";
            }, $topRecordings ?: []))
          . "\n\nBest regards,\nLatingles Academy\nWhatsApp: +1 754-364-4125";

    email_to_user(
        $student,
        $noreplyuser,
        $subject,   // e.g. '📚 1:1 Class Recordings'
        $text,      // plain text
        $html       // HTML
    );
}

// ---------- 6) Send to TEACHERS (teacher template) ----------
foreach ($teachers as $teacher) {
    // TEMP: force to your test inbox
    $teacher->email = 'rktech955@gmail.com';
...
<html>
  <body style='font-family: Arial, sans-serif; line-height: 1.6;'>
    ...
    <p>Here are the latest recordings for your recent 1:1 classes:</p>
    {$recHtml}
    <p>If any student needs additional support, feel free to share more resources with them.</p>
    ...
    $text = "Dear {$teachername},\n"
          . "Here are the latest recordings for your recent 1:1 classes:\n"
          . implode("\n", array_map(function($r) use($activityName) {
                $date = date('F d, Y', (int)$r->createdtime);
                $link = $r->webviewlink ?? '';
                return "{$activityName} – {$date}: {$link}";
