...
/**
 * Check a Moodle user's subscription state across providers.
 * Returns:
 *   [
 *     'state'          => 'active' | 'declined' | 'none',
 *     'provider'       => 'paypal'|'braintree'|'patreon'|null,
 *     'subscription_id'=> string|null,
 *     'price'          => float|null,
 *     'currency'       => string|null,
 *   ]
 */
// function membership_check_user_subscription(int $moodle_userid): array {
//     global $DB;

//     $best = ['state'=>'none','provider'=>null,'subscription_id'=>null,'price'=>null,'currency'=>null,'_t'=>0];

//     // Use the link-table so we query only identities tied to this Moodle user.
//     $links = $DB->get_records('payment_user_links', ['moodle_userid'=>$moodle_userid], 'last_seen_at DESC');
//     if (!$links) {
//         return array_intersect_key($best, array_flip(['state','provider','subscription_id','price','currency']));
//     }

//     foreach ($links as $l) {
//         $prov = $l->provider;
//         $pid  = $l->provider_user_id;

//         if ($prov === 'paypal') {
//             // pick the freshest row by a sensible touch timestamp
//             // ---- PayPal: pick the best status for this payer (ACTIVE > past_due > suspended > approved > approval_pending > canceled/expired > none)
//             $r = $DB->get_record_sql("
//                 SELECT s.*,
//                     COALESCE(s.status_update_time, s.last_payment_time, s.next_billing_time, s.create_time, s.updated_at) AS touchts,
//                     CASE
//                         WHEN UPPER(s.status)='ACTIVE' AND (COALESCE(s.failed_payment_count,0) > 0 OR COALESCE(s.outstanding_balance,0.0) > 0.0) THEN 70  -- past_due
//                         WHEN UPPER(s.status)='ACTIVE' THEN 80
//                         WHEN UPPER(s.status)='SUSPENDED' THEN 60
//                         WHEN UPPER(s.status) IN ('APPROVED','APPROVAL_PENDING') THEN 45
//                         WHEN UPPER(s.status) IN ('CANCELLED','EXPIRED') THEN 20
//                         ELSE 10
//                     END AS rankscore
//                 FROM {pp_subscriptions} s
//                 WHERE s.subscriber_payer_id = ?
//             ORDER BY rankscore DESC,
//                     COALESCE(s.status_update_time, s.last_payment_time, s.next_billing_time, s.create_time, s.updated_at) DESC
//                 LIMIT 1
//             ", [$pid]);

//             if ($r) {
//                 $state = 'none';
//                 $isactive   = ($r->status === 'ACTIVE') && ((int)($r->failed_payment_count ?? 0) <= 0) && ((float)($r->outstanding_balance ?? 0.0) <= 0.0);
//                 $isdeclined = ($r->status === 'ACTIVE') && ( ((int)($r->failed_payment_count ?? 0) > 0) || ((float)($r->outstanding_balance ?? 0.0) > 0.0) )
//                               || in_array($r->status, ['SUSPENDED'], true);

//                 if     ($isactive)   $state = 'active';
//                 elseif ($isdeclined) $state = 'declined';

//                 $cand = [
//                     'state'          => $state,
//                     'provider'       => 'paypal',
//                     'subscription_id'=> $r->subscription_id ?? null,
//                     'price'          => isset($r->last_payment_amount) ? (float)$r->last_payment_amount : null,
//                     'currency'       => $r->last_payment_currency ?? null,
//                     '_t'             => (int)strtotime($r->touchts ?? '1970-01-01 00:00:00'),
//                 ];
//                 $best = _membership_pick_better($best, $cand);
//             }
//         }

//         if ($prov === 'braintree') {
//             // ---- Braintree: Active > Past Due > Pending > (Canceled/Expired) > none
//             $r = $DB->get_record_sql("
//                 SELECT b.*,
//                     COALESCE(b.updated_at, b.paid_through_date, b.next_billing_date, b.billing_period_end, b.created_at) AS touchts,
//                     CASE b.status
//                         WHEN 'Active'   THEN 80
//                         WHEN 'Past Due' THEN 70
//                         WHEN 'Pending'  THEN 40
//                         WHEN 'Canceled' THEN 20
...
//                 WHERE b.customer_id = ?
//             ORDER BY rankscore DESC,
//                     COALESCE(b.updated_at, b.paid_through_date, b.next_billing_date, b.billing_period_end, b.created_at) DESC
//                 LIMIT 1
//             ", [$pid]);

//             if ($r) {
//                 $state = 'none';
//                 if ($r->status === 'Active')   $state = 'active';
//                 elseif ($r->status === 'Past Due') $state = 'Past Due';
//                 elseif ($r->status === 'Canceled') $state = 'Canceled';
...
//         if ($prov === 'patreon') {
//             // identity could be user_id OR member_id; match either
//           // ---- Patreon: active_patron > declined (incl. last_charge declined) > former_patron > none
//             $r = $DB->get_record_sql("
//                 SELECT p.*,
//                     COALESCE(p.last_charge_date, p.updated_at, p.pledge_relationship_start) AS touchts,
//                     CASE
//                         WHEN p.patron_status = 'active_patron' THEN 80
//                         WHEN p.patron_status = 'declined_patron' OR LOWER(COALESCE(p.last_charge_status,''))='declined' THEN 30
//                         WHEN p.patron_status = 'former_patron' THEN 20
//                         ELSE 10
//                     END AS rankscore
//                 FROM {pt_members} p
//                 WHERE p.user_id = ? OR p.member_id = ?
//             ORDER BY rankscore DESC,
//                     COALESCE(p.last_charge_date, p.updated_at, p.pledge_relationship_start) DESC
//                 LIMIT 1
//             ", [$pid, $pid]);

//             if ($r) {
//                 $state = 'none';
//                 if ($r->patron_status === 'active_patron')       $state = 'active';
//                 elseif ($r->patron_status === 'declined_patron' || (isset($r->last_charge_status) && strtolower($r->last_charge_status) === 'declined')) $state = 'declined';

//                 $price = null; $cur = null;
//                 if (isset($r->currently_entitled_amount_cents)) {
//                     $price = ((int)$r->currently_entitled_amount_cents) / 100.0;
//                     $cur   = $r->patron_currency ?? null;
//                 }

//                 $cand = [
//                     'state'          => $state,
//                     'provider'       => 'patreon',
//                     'subscription_id'=> $r->member_id ?? null,
//                     'price'          => $price,
//                     'currency'       => $cur,
//                     '_t'             => (int)strtotime($r->touchts ?? '1970-01-01 00:00:00'),
//                 ];
...
function membership_check_user_subscription(int $moodle_userid): array {
    ...
    if (!$links) {
        ...
        // Prefer a currently-valid ACTIVE manual row (date-limited if dates provided).
        $man = $DB->get_record_sql("
            SELECT *
          ...
          ORDER BY GREATEST(
                   COALESCE(end_date,0),
                   COALESCE(timemodified,0),
                   COALESCE(timecreated,0)
                 ) DESC
             LIMIT 1
        ", [$moodle_userid]);

        if ($man) {
            // Manual “active” grants access.
            return [
                'state'          => 'active',
                'provider'       => 'manual',
                'subscription_id'=> $man->subscriber_id ?? null,
                'price'          => isset($man->price) ? (float)$man->price : null,
                'currency'       => null, // no currency in manual table
            ];
        }else{
          return array_intersect_key($best, array_flip(['state','provider','subscription_id','price','currency']));
        }
    ...
    foreach ($links as $l) {
        ...
        if ($prov === 'patreon') {
         ...
         // ---------------- MANUAL FALLBACK ----------------
    // Only consider manual if we did NOT find an ACTIVE provider above.
    if ($best['state'] !== 'active') {
        ...
        $man = $DB->get_record_sql("
            SELECT *
             ...
             WHERE userid = ?
               AND LOWER(status) = 'active'
               AND (start_date IS NULL OR start_date = 0 OR start_date <= ?)
               AND (end_date   IS NULL OR end_date   = 0 OR end_date   >= ?)
          ORDER BY GREATEST(
                   COALESCE(end_date,0),
                   COALESCE(timemodified,0),
                   COALESCE(timecreated,0)
        ...
        // If there is a manual record but it’s inactive/expired we do NOT downgrade
        // anything; the best from gateways stands (declined/none).
    }
    }
...
/** Internal: prefer ACTIVE over DECLINED over NONE; on tie, pick newer touch time; on tie, keep existing. */
function _membership_pick_better(array $best, array $cand): array {
    static $rank = ['active'=>3, 'declined'=>2, 'none'=>1];
    $rb = $rank[$best['state']] ?? 0;
    $rc = $rank[$cand['state']] ?? 0;

    if    ($rc > $rb) return $cand;
    elseif($rc < $rb) return $best;

    // same class; prefer newer
    if (($cand['_t'] ?? 0) > ($best['_t'] ?? 0)) return $cand;

    // equal; if best has no price but cand has, take cand (useful for 'none' -> pick a price for pay page)
    $bestHasPrice = isset($best['price']) && $best['price'] !== null;
    $candHasPrice = isset($cand['price']) && $cand['price'] !== null;
    if (!$bestHasPrice && $candHasPrice) return $cand;
...
/**
 * Check if a user has either the student or teacher role.
 *
 * @param int $userid Moodle user ID
 * @return bool True if user has student (5) or teacher (3) role, false otherwise.
 */
/**
 * Check if a user has only the student role (role ID 5).
 *
 * @param int $userid Moodle user ID
 * @return bool True if user has only the student role, false otherwise.
 */
