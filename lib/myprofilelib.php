...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (!isset($hiddenfields['phone1'])) {
        $userphone = $DB->get_record('user',array('id' => $user->id));
        $phone = $user->phone1;
            if($user->phone1 == ''){
            $phone = 'not number';
        }
        $node = new core_user\output\myprofile\node('contact', 'phone1', get_string('phone'), null, null,
        $phone);
        $tree->add_node($node);
    }
    ...
    if (!is_user_siteadmin($USER->id)) { 
        // COHORTS
           if($USER->id != $user->id){
            $history = $DB->get_record('history_cohort2', array('idUser' => $user->id));

            // Crear el array de opciones vacías, y luego añadir las cohortes que existen en $history
            $options = [
                '' => 'View Cohorts', // Opción por defecto
            ];
        
            // Agregar las cohortes que existen en el objeto $history
            if (!empty($history->onecohort)) {
                $options['cohort_1'] = 'First Cohort : ' . $history->onecohort;
            }
            if (!empty($history->twocohort)) {
                $options['cohort_2'] = 'Second Cohort : ' . $history->twocohort;
            }
            if (!empty($history->threecohort)) {
                $options['cohort_3'] = 'Third Cohort : ' . $history->threecohort;
            }
            if (!empty($history->fouthcohort)) {
                $options['cohort_4'] = 'Fourth Cohort : ' . $history->fouthcohort;
            }
            
            if (!empty($history->fifthcohort)) {
                $options['cohort_5'] = 'Fifth Cohort : ' . $history->fifthcohort;
            }
            if (!empty($history->sixthcohort)) {
                $options['cohort_6'] = 'Sixth Cohort : ' . $history->sixthcohort;
            }
        
            // Determinar cuál de las cohortes es la seleccionada
            $selected_cohort = $history->onecohort; // O tomar cualquier otra si lo prefieres
            if (!in_array($selected_cohort, array_keys($options))) {
                $selected_cohort = ''; // Si no es una opción válida, poner la opción por defecto
            }
        
            // Crear el HTML para el select
            $select_html = '<select name="cohort1" id="cohort1_select">';
            foreach ($options as $value => $label) {
                $selected = ($value === $selected_cohort) ? ' selected' : '';
                $select_html .= '<option disabled value="' . $value . '"' . $selected . '>' . $label . '</option>';
            }
            $select_html .= '</select>';
        
            // Crear el nodo con el HTML del select
            $node = new core_user\output\myprofile\node('contact', 'cohort1', 'History Cohorts', null, null, $select_html);
            $tree->add_node($node);
...
// Consulta SQL para obtener la cohorte actual del usuario
        $sql = "
        SELECT c.id AS cohort_id, c.name AS cohort_name, c.shortname AS cohort_shortname, c.idnumber AS idnumber
        FROM {cohort} c
        ...
        $current_cohort = $DB->get_record_sql($sql, array(
        'userid' => $user->id,
        'current_time' => time()
        ));
        
        if ($current_cohort) {
            $content = $current_cohort->cohort_name . ' | <b>' . $current_cohort->idnumber .'</b>';
            $node = new core_user\output\myprofile\node('contact', 'currentCohort', 'Current Cohort', null, null, $content);
            $tree->add_node($node);
        } 
