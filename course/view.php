...
// If section is 0 (General), find the first available section
if (empty($section) || $section == 0) {
    $firstsection = $DB->get_field_sql("
        SELECT section 
    ...
    if ($firstsection) {
        $section = $firstsection; // Assign first available section
    }
...
$sectionid = $DB->get_field('course_sections', 'id', [
    ...
    // Step 1: Check if a subsection exists with specific names (case-insensitive)
$mod = $DB->get_record_sql("
SELECT cm.id, cm.instance, cm.module 
...
if($mod){
// Initialize next section search
if (!is_null($section)) {
    $urlparams['section'] = $section;
}
}else{
    ...
    // Assume you have $section (which is the section number) and $courseid

// Step 1: Check if a subsection exists with specific names (case-insensitive)
$subsection = $DB->get_record_sql("
    SELECT id, name 
    FROM {course_sections} 
    WHERE course = :courseid 
    AND section > (SELECT section FROM {course_sections} WHERE id = :sectionid)
    AND LOWER(name) IN ('topic', 'homework', 'quizzes', 'practice')
    ORDER BY section ASC 
    LIMIT 1",
    ['courseid' => $course->id, 'sectionid' => $sectionid]
);

if ($subsection) {
        // Initialize next section search

        if(strtolower($subsection->name) == 'topic')
        {
            $sectionid = $DB->get_field('course_sections', 'id', [
                'course' => $course->id,
        ...
        else if(strtolower($subsection->name) == 'homework')
        {
            $sectionid = $DB->get_field('course_sections', 'id', [
                'course' => $course->id,
        ...
        else if(strtolower($subsection->name) == 'quizzes')
        {
            $sectionid = $DB->get_field('course_sections', 'id', [
                'course' => $course->id,
                'section' => ((int)$section + 0)
        ...
        else if(strtolower($subsection->name) == 'practice')
        {
            $sectionid = $DB->get_field('course_sections', 'id', [
                'course' => $course->id,
                'section' => ((int)$section + 0)
