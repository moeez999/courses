...
function add_cohort_restriction($courseid, $sectionid, $cohortid) {
   ...
   require_once($CFG->dirroot . '/course/lib.php'); // see note below

    $course  = get_course($courseid);
    $section = $DB->get_record('course_sections', ['id' => $sectionid], '*', MUST_EXIST);

    // Build cohort availability (same as Moodle UI)
    $availability = [
        'op' => '&',
        'c'  => [
            [
    ...
    // ✅ Pass changes via $data (THIS is the key)
    $data = [
        'availability' => json_encode($availability, JSON_UNESCAPED_SLASHES),
    ];

    // ✅ Correct API usage (3 params)
...
function create_course_section($courseid, $cohortShortName) {
    ...
    if ($section->id) {
         global $CFG;           // <-- needed inside functions
    require_once($CFG->dirroot . '/course/lib.php'); // see note below
